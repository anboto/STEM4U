...
class AsymptoticConvergence2 {
public:
	double Get(const UVector<double> &y, double ratioY, double ratioY2, double minr2, double rangeRatio, double &r2) {
		ASSERT(x.size() == y.size());
		...
		// Remove big outliers from shape with ratioY
		for (int i = yy.size()-1; i >= 0; --i) {
			if (abs(yy[i] - asy.f(xx[i]))/asy.f(xx[i]) > ratioY) {
				xx.Remove(i);		
				yy.Remove(i);
				if (ww.size() > 0)
					Remove(ww, i);
		...
		// From idfirst to Last are the values to be fitted always
		double rangex = Last(xx) - First(xx);
		double first = Last(xx) - rangex*rangeRatio;
		int idfirst;
		for (int i = xx.size()-1; i >= 0; --i) {
		...
		while (true) {
			...
			if (r2 > minr2)
				return asy.GetCoeff(0);
			bool removed = false;
			for (int i = xx.size()-1; i > idfirst; --i) {
				if (abs(yy[i] - asy.f(xx[i]))/asy.f(xx[i]) > ratioY2) {
					xx.Remove(i);		
					yy.Remove(i);
					if (ww.size() > 0)
						Remove(ww, i);
	...
	void SetX(const UVector<double> &_x) {			
		x = clone(_x);
		w.resize(x.size());
		
		// Fitting weight is proportional to the area around each value
		First(w) = x[1] - x[0];						// It is supposed the same area at the left of the first
		Last(w)  = x[x.size()-1] - x[x.size()-2];	// It is supposed the same area at the right of the last
		for (int i = 1; i < w.size()-1; ++i)
			w[i] = (x[i+1] - x[i-1])/2;				//	((w[i] - w[i-1]) + (w[i+1] - w[i]))/2
		
		// However, it is also inversely proportional to the distance to zero
